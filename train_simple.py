...
def build_model(df_columns_len):
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(500, kernel_regularizer=tf.keras.regularizers.l2(0.001), activation=tf.nn.relu),
        tf.keras.layers.Dropout(0.2),
        tf.keras.layers.Dense(500, kernel_regularizer=tf.keras.regularizers.l2(0.001), activation=tf.nn.relu),
        tf.keras.layers.Dropout(0.2),
        tf.keras.layers.Dense(500, kernel_regularizer=tf.keras.regularizers.l2(0.001), activation=tf.nn.relu),
...
def train_model(train_data,train_label,val_data,val_label,target_name,):
    ...
    history = model.fit(train_data,
        ...
        epochs=30,
        batch_size=train_data.shape[1]//3,
        callbacks=callbacks)
...
def keras_train(target_name):
    ...
    predict_proba_results=np.argmax(predict_prob,axis=1)
